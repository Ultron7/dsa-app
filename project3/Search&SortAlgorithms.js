...
function getInput() {
  ...
  let e = "";
  for (let y = 0; y < displayArray.length; y++) {
    e += ` ${displayArray[y]} `;
  }
...
function SelectionSort(array) {
  for (let i = 0; i < array.length; i++) {
    ...
    for (let j = i + 1; j < array.length; j++) {
      //it updates when ever it finds number less than the one at index i

      if (array[j] < array[testIndex]) testIndex = j;
    }
    if (testIndex != i) swap(array, testIndex, i);
  }
  return array;
}
...
function measurePerformance() {
  ...
  let inde = binary(tempArray, theNumber, 0, tempArray.length - 1);
  let endtym = Date.now();
  let timeTaken = endtym - starttym;
  alert(
    `${inde} and Binary Search Algorithm has taken ${timeTaken} milliseconds to find it.`
  );
}
...
function measurePerformanceDL() {
  let theNumber = Number(numsearch.value);
  let starttym = Date.now();
  let inde = directlinear(tempArray, theNumber, tempArray.length);
  let endtym = Date.now();
  let timeTaken = endtym - starttym;
  console.log(tempArray);
  alert(
    `${inde} and Direct Linear Search Algorihm has taken ${timeTaken} milliseconds to find it.`
  );
}
...
function measurePerformanceIL() {
  let theNumber = Number(numsearch.value);
  let starttym = Date.now();
  let inde = undirectlinear(tempArray, theNumber, tempArray.length);
  let endtym = Date.now();
  let timeTaken = endtym - starttym;
  alert(
    `${inde} and Indirect Linear Search Algorihm has taken ${timeTaken} milliseconds to find it.`
  );
}
